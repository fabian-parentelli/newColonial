...
const postProduct = async (body, imagesUrl) => {
    ...
    if (!result) throw new ErrorCustom('Error al guardar el producto');
...
const getProducts = async ({ page = 1, limit = 12, brand, id, category, subcategory, active,
    ...
    if (!result) throw new ErrorCustom('Error al traer los productos');
    return { status: 'success', result };
};

const putProductImg = async (body, imagesUrl) => {
    const product = await productRepository.getById(body.id);
    if (!product) throw new ErrorCustom('Error al tarer el producto');
    const result = await productRepository.update({ ...product, img: imagesUrl[0] });
    if (!result) throw new ErrorCustom('Error al actulizar el producto');
...
const putOpportinity = async ({ password }, user) => {
    ...
    if (!products) throw new ErrorCustom('Error al tarer las oportunidades');
    const notid = [];
    ...
    if (!productRandom) throw new ErrorCustom('Error al tarer los productos random');
...
const putProduct = async (body) => {
    ...
    if (product.family && (body.price !== product.price || body.discount !== product.discount || body.box !== product.box || body.cost !== product.cost)) {
        ...
        if (!products) throw new ErrorCustom('Error al tarer los productos');
        const prodArray = [];
        for (const prod of products) {
            ...
            if (!prodUpdate) throw new ErrorCustom('Error al actualizar el producto');
